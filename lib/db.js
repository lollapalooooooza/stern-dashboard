...
const THEMATIC_SEED_VERSION = "2026-04-20-positions-balance-v1";
...
function shouldResetTrackerStart(rows, group) {
  if (group !== "thematic") return false;
...
function normalizeWeeklyHistory(rows, group = "thematic") {
  const cleaned = rows
    .map((row, index) => ({
  ...
  const resetToTrackerStart = shouldResetTrackerStart(sorted, group);
  const trimmed = resetToTrackerStart
    ? sorted.filter((row) => row.date && row.date >= THEMATIC_TRACKER_START_DATE)
    : sorted;
  const zeroIndexedWeeks = (group === "thematic" && trimmed[0]?.date === THEMATIC_TRACKER_START_DATE)
...
function normalizeDailyHistory(rows, group = "thematic") {
  const cleaned = rows
    .map((row, index) => ({
      date: row.date ? String(row.date).slice(0, 10) : "",
  ...
  const trimmed = resetToTrackerStart
    ? sorted.filter((row) => row.date && row.date >= THEMATIC_TRACKER_START_DATE)
    : sorted;
  const trackerBaseline = group === "thematic" && trimmed[0]?.date === THEMATIC_TRACKER_START_DATE;
...
function summarizeSeedHoldings(rows) {
  ...
  const activeStocks = active.filter((row) => row[3] !== "Benchmark");
...
async function seedThematic(db, group) {
  ...
  const normalizedDailyHistory = normalizeDailyHistory(THEMATIC_WORKBOOK_DAILY_HISTORY, group);
  const dh = normalizedDailyHistory.map((row) => [
    row.date,
    row.portfolioValue,
    row.benchmarkValue,
    row.portfolioReturn,
  ...
  const normalizedWeeklyHistory = normalizeWeeklyHistory(THEMATIC_WORKBOOK_WEEKLY_HISTORY, group);
  const wh = normalizedWeeklyHistory.map((row) => [
    row.week,
    row.date,
    row.portfolioReturn,
    row.benchmarkReturn,
...
export async function getWeeklyHistory(group) {
  ...
  const normalized = normalizeWeeklyHistory(res.rows, group);
...
export async function saveWeeklyHistory(rows, group) {
  await init(group);
  await writeWeeklyHistory(normalizeWeeklyHistory(rows, group), group);
}
...
export async function getDailyHistory(group) {
  ...
  const normalized = normalizeDailyHistory(res.rows, group);
...
export async function saveDailyHistory(rows, group) {
  await init(group);
  await writeDailyHistory(normalizeDailyHistory(rows, group), group);
}
