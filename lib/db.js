...
import {
  THEMATIC_WORKBOOK_ACTIVE_OVERRIDES,
  THEMATIC_WORKBOOK_BENCHMARK_CURRENT_PRICE,
  THEMATIC_WORKBOOK_BENCHMARK_SHARES,
  THEMATIC_WORKBOOK_WEEKLY_HISTORY,
} from "./thematicWorkbookData.js";

const GROUPS = ["thematic", "opportunistic", "systematic", "bond"];
const THEMATIC_SEED_VERSION = "2026-03-25-workbook-v3";
...
function holdingKey(theme, ticker) {
  return `${theme}|${ticker}`;
}

function buildWaterSeedRow(override, index) {
  return [
    `water-${index + 1}`,
    override.ticker,
    override.company || override.ticker,
    override.theme,
    override.theme,
    Number(override.buyPrice),
    Number(override.currentPrice),
    override.entryDate || "",
    "",
    Number(override.shares),
    0,
    0.1,
    "active",
    `${override.company || override.ticker} position`,
    1,
    0,
    0,
    0,
    Number(override.currentValue),
    Number(override.pnl),
    0,
    0,
    0,
...
function buildThematicActiveRow(existingByKey, override, index) {
  const template = existingByKey.get(holdingKey(override.theme, override.ticker));
  const row = template ? [...template] : buildWaterSeedRow(override, index);
  row[0] = template?.[0] || row[0];
  row[1] = override.ticker;
  row[2] = template?.[2] || override.company || override.ticker;
  row[3] = override.theme;
  row[4] = template?.[4] || override.theme;
  row[5] = Number(override.buyPrice);
  row[6] = Number(override.currentPrice);
  row[7] = override.entryDate || "";
  row[8] = "";
  row[9] = Number(override.shares);
  row[12] = "active";
  row[18] = Number(override.currentValue);
  row[19] = Number(override.pnl);
  row[20] = 0;
  row[21] = 0;
  row[22] = 0;
  row[23] = 0;
...
function buildThematicSeedHoldings() {
  const existingByKey = new Map(
    THEMATIC_HOLDINGS.map((row) => [holdingKey(row[3], row[1]), row])
  );
  const benchmarkRow = [...existingByKey.get(holdingKey("Benchmark", "SPY"))];
  benchmarkRow[6] = Number(THEMATIC_WORKBOOK_BENCHMARK_CURRENT_PRICE);
  benchmarkRow[9] = Number(THEMATIC_WORKBOOK_BENCHMARK_SHARES);
  benchmarkRow[10] = 0.341;
  benchmarkRow[12] = "active";
  benchmarkRow[13] = "S&P 500 core";
  benchmarkRow[18] = Number(THEMATIC_WORKBOOK_BENCHMARK_CURRENT_PRICE) * Number(THEMATIC_WORKBOOK_BENCHMARK_SHARES);
  benchmarkRow[19] = (Number(THEMATIC_WORKBOOK_BENCHMARK_CURRENT_PRICE) - Number(benchmarkRow[5])) * Number(THEMATIC_WORKBOOK_BENCHMARK_SHARES);
  benchmarkRow[8] = "";

  const activeRows = THEMATIC_WORKBOOK_ACTIVE_OVERRIDES.map((override, index) =>
    buildThematicActiveRow(existingByKey, override, index)
  );
  const exitedRows = THEMATIC_HOLDINGS.filter((row) => row[12] === "exited").map((row) => [...row]);
  return [benchmarkRow, ...activeRows, ...exitedRows];
}

function summarizeSeedHoldings(rows) {
  const active = rows.filter((row) => row[12] === "active");
  const exited = rows.filter((row) => row[12] === "exited");
  const totalVal = active.reduce((sum, row) => sum + Number(row[9]) * Number(row[6]), 0);
  ...
  const totalCostBasis = active.reduce((sum, row) => sum + Number(row[9]) * Number(row[5]), 0) + exited.reduce((sum, row) => sum + Number(row[21] || (Number(row[9]) * Number(row[5]))), 0);
  return {
