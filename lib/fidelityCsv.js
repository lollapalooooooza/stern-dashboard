...
export function buildHoldingsFromFidelityCsv(csvText, existingHoldings, options = {}) {
  ...
  if (headerIndex < 0) throw new Error("Could not find a Fidelity positions header row in this CSV.");
  ...
  if (!importedPositions.length) throw new Error("The CSV did not contain any active positions to import.");
