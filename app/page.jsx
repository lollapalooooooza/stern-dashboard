...
function OverviewPage({ holdings, settings, weeklyHistory }) {
  ...
  return <div className="space-y-6">
    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
      ...
      <StatCard label="Realized PnL" value={sub={fmt.usd(totalRealizedPnl)} fmt.pct(startingVal>0?totalRealizedPnl/startingVal:0)} trend={totalRealizedPnl>=0?"up":"down"} color={totalRealizedPnl>=0?"text-emerald-700":"text-red-600"} icon={LogOut} tooltip={`${exited.length} exited positions\nvs starting value ${fmt.usd(startingVal)}`}/>
      <StatCard label="Total Return" value={sub={fmt.usd(totalPnlFromBalance)} fmt.pct(cumReturn)} trend={cumReturn>=0?"up":"down"} color={cumReturn>=0?"text-emerald-700":"text-red-600"} icon={BarChart3} tooltip={`From account balances\nStart: ${fmt.usd(startingVal)}\nNow: ${fmt.usd(totalVal)}\nMatches cumulative chart`}/>
