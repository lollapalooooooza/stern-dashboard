export async function POST(request) {
  try {
    ...
    for (const q of queries) {
      try {
        const resp = await fetch(`https://news.google.com/rss/search?q=${encodeURIComponent(q)}&hl=en-US&gl=US&ceid=US:en`, { headers:{"User-Agent":"Mozilla/5.0"}, signal:AbortSignal.timeout(10000) });
        if (!resp.ok) continue;
        ...
        while ((m=re.exec(xml))!==null && cnt<5) {
          ...
          const matched=tickers.filter(t=>upper.includes(` ${t.toUpperCase()} `));
    ...
    const seen=new Set();
    return Response.json({news:allNews.filter(n=>{const k=n.title.substring(0,50).toLowerCase();if(seen.has(k))return false;seen.add(k);return true}).slice(0,10)});
  } catch(e) { return Response.json({news:[],error:e.message},{status:500}); }
}
