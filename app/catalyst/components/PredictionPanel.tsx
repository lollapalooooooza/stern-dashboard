...
export default function PredictionPanel({ symbol }: Props) {
  ...
  useEffect(() => {
    ...
    setError('');
    Promise.all([
      catalystApi.get(`predict/${symbol}/forecast?window=7`).then((res) => res.data as Forecast).catch(() => null),
      catalystApi.get(`predict/${symbol}/forecast?window=30`).then((res) => res.data as Forecast).catch(() => null),
    ])
      .then(([f7, f30]) => {
        setForecast7(f7);
        setForecast30(f30);
                ...
                {deep ? (
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      showToast('AI deep analysis started...', 'info');
                      catalystApi
                        .post('analysis/deep', { news_id: article.news_id, symbol })
                        .then((res) => {
